...
export function createLogger(options: LoggerOptions = {}) {
  ...
  if (options.pretty && !isTestEnvironment()) {
    // Keep this as a plain package name.
    // In webpack bundles, require.resolve("pino-pretty") can be rewritten to
    // a numeric module id, which then breaks pino transport target resolution.
    const prettyTarget = "pino-pretty";
  ...
  try {
    ...
    if (loggerConfig.transport) {
      console.warn(
        `pino transport init failed (${errorMessage}), falling back to standard logging`,
      );
      return pino(
        {
          ...loggerConfig,

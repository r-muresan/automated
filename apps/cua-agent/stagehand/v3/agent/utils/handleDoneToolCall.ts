...
function buildBaseDoneSchema(factory: typeof z) {
  return factory.object({
    reasoning: factory.string().describe('1 sentence summary of final result or issues'),
...
export async function handleDoneToolCall(options: {
  ...
  const factory = outputSchema ? getZFactory(outputSchema as StagehandZodSchema) : z;
  ...
  const outputInstructions = outputSchema
    ? `\n\nThe user also requested the following information from this task. Provide it in the "output" field:\n${JSON.stringify(
        Object.fromEntries(
          Object.entries(outputSchema.shape).map(([key, value]: [string, StagehandZodSchema]) => [
            key,
...
1. A very short summary of the current state of the task

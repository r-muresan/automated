...
export class OpenAIClient extends LLMClient {
  ...
  constructor({
    ...
    const openAIOptions: OpenAIOptions = {
      ...
      project: (clientOptions as { project?: string } | undefined)?.project,
    };
  ...
  async createChatCompletion<T = LLMResponse>({
    ...
    let options: Partial<ChatCompletionOptions> & { messages: ChatMessage[] } = {
      ...optionsInitial,
    ...
    if (isToolsOverridedForO1) {
      const toolCallContent = response.choices[0]?.message?.content;
      try {
        if (typeof toolCallContent !== "string") {
          throw new Error("Expected tool call response content to be a string.");
        }
    ...
    if (options.response_model) {
      ...
      if (typeof extractedData !== "string") {
        throw new CreateChatCompletionResponseError(
          "Expected response content to be a string for schema parsing.",
        );
      }
